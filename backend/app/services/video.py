...
async def generate_video(request: VideoGenerateRequest):
    ...
    try:
        ...
        if request.test_mode:
            ...
            # 如果请求中包含已确认的故事段落，直接使用它们
            if request.story_segments:
                logger.info(f"Using confirmed story segments from request,{request.story_segments}")
                scenes = request.story_segments
                # 如果段落中没有图片URL，则生成图片
                for scene in scenes:
                    if not scene.url:
                        try:
                            ...
                            scene.url = llm_service.generate_image(
                                prompt=image_prompt,
                            ...
                            logger.error(f"Failed to generate image for scene: {e}")
                            scene.url = None
            else:
                # 如果没有提供故事段落，则生成新的故事
            ...
            story_file = os.path.join(task_dir, "story.json")
            # 根据数据来源选择正确的迭代对象
            scene_data_list = story_list if not request.story_segments else scenes
            for i, scene in enumerate(scene_data_list, 1):
                # 处理不同类型的场景对象（字典或StoryScene）
