...
class LLMService:
    ...
    async def generate_story(self, request: StoryGenerationRequest) -> List[Dict[str, Any]]:
        ...
        messages = [
            ...
            {"role": "user", "content": await self._get_story_prompt(request.story_prompt, request.language, request.segments, request.story_setting)}
        ]
    ...
    def generate_image(self, *, prompt: str, image_llm_provider: str = None, image_llm_model: str = None, resolution: str = "1024x1024",story_setting: str = None) -> str:
        ...
        try:
            # 添加安全提示词
            safe_prompt = f"Create a safe, family-friendly illustration. {story_setting},{prompt} The image should be appropriate for all ages, non-violent, and non-controversial."
            #logger.info(f"Generated safe_prompt with story_setting: {safe_prompt}")
    ...
    async def generate_story_with_images(self, request: StoryGenerationRequest) -> List[Dict[str, Any]]:
        ...
        """
        # 先生成故事
        #logger.info(f"generate_story_with_images received story_setting: {request.story_setting}")
        ...
        for segment in story_segments:
            try:
                ...
                if request.story_setting:
                    image_prompt = f"{request.story_setting},{image_prompt}"
                #logger.info(f"Generating image with story_setting: {image_prompt}")
                #logger.info(f"Generating image with prompt: {segment['image_prompt']}")

                image_url = self.generate_image(prompt=image_prompt, resolution=request.resolution, image_llm_provider=request.image_llm_provider, image_llm_model=request.image_llm_model, story_setting=request.story_setting)
    ...
    async def _get_story_prompt(self, story_prompt: str = None, language: Language = Language.CHINESE_CN, segments: int = 3, story_setting: str = None) -> str:
        """
        生成故事提示词
        ...
        #logger.info(f"_get_story_prompt received story_setting: {story_setting}")

        languageValue = LANGUAGE_NAMES[language]
        if story_prompt:
            base_prompt = f"讲一个故事，主题是：{story_prompt}"
            if story_setting:
                base_prompt += f"，故事设定是：{story_setting}"
                logger.info(f"Added story_setting to base_prompt: {base_prompt}")
        else:
            base_prompt = "讲一个故事"
            if story_setting:
                base_prompt += f"，故事设定是：{story_setting}"
